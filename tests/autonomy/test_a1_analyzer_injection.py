...
def test_a1_analyzer_injection_does_not_infer():
    ...
    snap_normal = StateSnapshot(
        state_version=1, simulation_tick=100, simulation_time=100.0,
        gcs_position=(0.0, 0.0), uavs={"uav_1": UAVState(id="uav_1", position_xy=(50.0, 0.0)), "uav_2": UAVState(id="uav_2", position_xy=(90.0, 0.0))}, tasks={}
    )
    ...
    snap_degrade = StateSnapshot(
        state_version=2, simulation_tick=200, simulation_time=200.0,
        gcs_position=(0.0, 0.0), uavs={"uav_1": UAVState(id="uav_1", position_xy=(50.0, 0.0)), "uav_2": UAVState(id="uav_2", position_xy=(90.0, 0.0))}, tasks={}
    )
    net_degrade = analyzer.analyze(snap_degrade)
    extracted_degrade = allocator._get_comm_analyzer(net_degrade)
    # The analyzer still has 100m range (no wrong inference)
    assert extracted_degrade.config.max_range == 100.0
    # The analyzer natively knows about the quality multiplier via its scenario conditions
    assert len(extracted_degrade.scenario_conditions) == 2

    # 3. Outage (t=300)
    snap_outage = StateSnapshot(
        state_version=3, simulation_tick=300, simulation_time=300.0,
        gcs_position=(0.0, 0.0), uavs={"uav_1": UAVState(id="uav_1", position_xy=(50.0, 0.0)), "uav_2": UAVState(id="uav_2", position_xy=(90.0, 0.0))}, tasks={}
    )
