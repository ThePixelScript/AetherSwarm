...
def test_articulation_regression():
    ...
    # Check uav_2 allocation feasibility
...
def test_already_disconnected_peer_regression():
    # uav_4 is already disconnected
    uavs = (
        {'id': 'uav_1', 'position': [80.0, 0.0], 'role': 'IDLE'},
        {'id': 'uav_2', 'position': [160.0, 0.0], 'role': 'IDLE'},
        {'id': 'uav_4', 'position': [500.0, 0.0], 'role': 'IDLE'}, # Completely disconnected
    )
    # Task at [160, 40]
    tasks = ({
        'id': 'poi_1',
    ...
    # uav_2 moves to [160, 40]. Dist to uav_1 [80, 0] is sqrt(80^2 + 40^2) = 89.4m (Connected)
    uav_2 = snap.uavs['uav_2']
    task = snap.tasks['poi_1']
...
def test_dead_zone_regression():
    # Task is outside comm range. Even if it doesn't disconnect peers, candidate itself loses comms!
    uavs = (
        {'id': 'uav_1', 'position': [80.0, 0.0], 'role': 'IDLE'},
    )
    tasks = ({
        'id': 'poi_1',
        'position': [200.0, 0.0], # 200m from GCS, no relays!
        'spawn_time': 0.0,
    },)
    scen = create_base_scenario(uavs, tasks)
    analyzer = BaselineCommunicationAnalyzer(config=scen.communication)
