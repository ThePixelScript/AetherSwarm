...
def test_a1_preserves_telemetry_under_controlled_outage():
    # We load the exact yaml we generated
    ...
    # --- Run A0 ---
    a0_analyzer = BaselineCommunicationAnalyzer(config=base_scenario.communication, scenario_conditions=conds)
    a0_runner = MissionRunner(
        scenario=base_scenario, seed=2026,
    ...
    # --- Run A1 ---
    a1_analyzer = BaselineCommunicationAnalyzer(config=base_scenario.communication, scenario_conditions=conds)
